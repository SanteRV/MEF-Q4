...
(TEORÍA OK_PROGRAMA.docx, cap. 01.01.03), por el método de la FUNCIÓN DE
DESPLAZAMIENTOS, en su mismo orden y con sus mismas fórmulas:

    1. Polinomio de 12 términos para w(x, y)                      (ec. 3.11)
       w = α1 + α2·x + α3·y + α4·x² + α5·xy + α6·y² + α7·x³
           + α8·x²y + α9·xy² + α10·y³ + α11·x³y + α12·xy³
    2. w = [P]·[α]  con P = fila de los 12 monomios               (ec. 3.13)
    3. δ = [C]·[α]  evaluando (w, θx, θy) en los 4 nodos          (ec. 3.14)
       con θx = ∂w/∂y (ec. 3.3) y θy = -∂w/∂x (ec. 3.4)
    4. Deformaciones:  ε = -z·[Q]·[α]                             (ec. 3.16)
       (curvaturas: ∂²w/∂x², ∂²w/∂y², 2·∂²w/∂x∂y)
    5. Matriz B:       B = -z·[Q]·[C⁻¹]                           (ec. 3.17)
    6. Rigidez:  K^e = [C⁻¹]ᵀ·[∫∫ Qᵀ·D·Q dx dy]·[C⁻¹]·(t³/12)     (ec. 3.18)
       con D del estado plano de esfuerzos (ec. 3.9)
    7. Carga distribuida q → F nodal = q·A/4 en los GDL w         (ec. 3.19)
...
def P_row(x: float, y: float) -> np.ndarray:
    """Fila P de los 12 monomios del polinomio de w (ec. 3.12).
...
def dP_dx_row(x: float, y: float) -> np.ndarray:
    """Derivada ∂P/∂x — se usa (negada) para la fila de θy en C (ec. 3.4)."""
    return np.array([
...
def dP_dy_row(x: float, y: float) -> np.ndarray:
    """Derivada ∂P/∂y — es la fila de θx en C (ec. 3.3)."""
    return np.array([
...
def Q_matrix(x: float, y: float) -> np.ndarray:
    """Matriz Q (3×12): curvaturas del polinomio w (ec. 3.16).

    Para qué sirve: las deformaciones de flexión son proporcionales a las
    curvaturas de la placa,  ε = -z·[∂²w/∂x²; ∂²w/∂y²; 2·∂²w/∂x∂y]
    (ec. 3.8). Q expresa esas curvaturas en función de los coeficientes α:
...
class PlateElement:
    ...
    su matriz de rigidez K^e (12×12, ec. 3.18) y recuperar curvaturas,
    ...
    def __post_init__(self) -> None:
        ...
        # La formulación por función de desplazamientos (ec. 3.11-3.18)
    ...
    def C_matrix(self) -> np.ndarray:
        ...
        desplazamientos nodales en coeficientes: α = C⁻¹·δ (ec. 3.15).

        Por cada nodo i aporta 3 filas:
            fila w  :  P(xi, yi)                (ec. 3.13)
            fila θx :  ∂P/∂y (xi, yi)           (θx = ∂w/∂y, ec. 3.3)
            fila θy : -∂P/∂x (xi, yi)           (θy = -∂w/∂x, ec. 3.4)
        """
        C = np.zeros((12, 12))
    ...
    def D_matrix(self) -> np.ndarray:
        """Matriz constitutiva D (3×3) del estado plano de esfuerzos (ec. 3.9).
    ...
    # ---------- 7. Vector de fuerzas por carga distribuida (ec. 3.19) ----------
    def load_vector_uniform(self, q: float) -> np.ndarray:
        """Vector de fuerzas nodales para una carga q uniforme (N/m²).

        1er caso del documento (ec. 3.19): la carga total q·A se reparte
    ...
    def strains_stresses_at(self, x: float, y: float, z: float,
        ...
        ε = B·δ con B = -z·Q·C⁻¹ (ec. 3.17) y σ = D·ε (ec. 3.10).
    ...
    def w_at(self, x: float, y: float, displacements_12: np.ndarray) -> float:
        """Desplazamiento transversal w interpolado en un punto (ec. 3.15)."""
        C_inv = np.linalg.inv(self.C_matrix())
